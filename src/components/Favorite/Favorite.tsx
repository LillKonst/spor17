...
//             <Link
//               to={`/produkt/${product.handle}`}
//               className="p-2 px-3 text-sm rounded border-2 border-ctaPink w-[195px] text-center"
//             >
//               Se detaljer
//             </Link>
//           </div>
...
export default function DagensKort() {
  const [product, setProduct] = useState<Product | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    fetchAllProducts()
      .then((products) => {
        ...
        const todaysProduct =
          products[dayIndex % products.length];

        setProduct(todaysProduct);
      })
      .catch(() => setProduct(null))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <p>Laster dagens kort…</p>;
  if (!product) return <p>Kunne ikke hente dagens kort</p>;
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="self-center xs:h-full flex flex-col my-3 xs:justify-between lg:mx-10">
          ...
          <div className="flex flex-col gap-2 max-w-[200px] self-center">
            ...
            <Link
              to={`/produkt/${product.handle}`}
              className="p-2 px-3 text-lg sm:text-xl rounded border-2 border-ctaPink w-[179px] sm:w-[195px] text-center"
            >
              Se detaljer
            </Link>
          </div>
